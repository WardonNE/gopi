...
type SyncLinkedHashMap[K, V comparable] struct {
	mu      *sync.RWMutex
	hashmap *LinkedHashMap[K, V]
}

func NewSyncLinkedHashMap[K, V comparable]() *SyncLinkedHashMap[K, V] {
...
func (m *SyncLinkedHashMap[K, V]) ContainsValue(value V) bool {
	m.mu.RLock()
	defer m.mu.RUnlock()
