...
func TestSyncLinkedQueue_ToArray(t *testing.T) {
	q := NewSyncLinkedQueue[int]()
	q.Enqueue(1)
	q.Enqueue(2)
	q.Enqueue(3)
...
func TestSyncLinkedQueue_FromArray(t *testing.T) {
	q := NewSyncLinkedQueue[int]()
	q.FromArray([]int{1, 2, 3, 4, 5})
	assert.Equal(t, []int{1, 2, 3, 4, 5}, q.ToArray())
}

func TestSyncLinkedQueue_Count(t *testing.T) {
	q := NewSyncLinkedQueue[int]()
	q.FromArray([]int{1, 2, 3, 4, 5})
...
func TestSyncLinkedQueue_Enqueue(t *testing.T) {
	q := NewSyncLinkedQueue[int]()
	q.Enqueue(1)
	q.Enqueue(2)
	q.Enqueue(3)
...
func TestSyncLinkedQueue_Dequeue(t *testing.T) {
	q := NewSyncLinkedQueue[int]()
	value, ok := q.Dequeue()
	assert.Zero(t, value)
	assert.False(t, ok)
